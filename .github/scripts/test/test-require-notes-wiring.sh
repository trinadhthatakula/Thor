...
check('"$REQUIRE_NOTES" = "true"' in body, "pre-flight gate does not branch on require_notes")
check('"$TELEGRAM" = "true"' in body, "pre-flight gate requires telegram.md unconditionally")
check(body.find('"$REQUIRE_NOTES" = "true"') < body.find("--require github.md"),
...
check(env.get("REQUIRE_NOTES") == "${{ inputs.require_notes }}", "gate step does not read inputs.require_notes")
...
prep_body = prep["run"]
g = prep_body.find('"$REQUIRE_NOTES" = "true"')
...
check((prep.get("env") or {}).get("REQUIRE_NOTES") == "${{ inputs.require_notes }}",
      "Prepare Release Notes does not read inputs.require_notes")

tg = by_name["Send APK to Telegram"]
tg_body = tg["run"]
g = tg_body.find('"$REQUIRE_NOTES" = "true"')
...
check((tg.get("env") or {}).get("REQUIRE_NOTES") == "${{ inputs.require_notes }}",
