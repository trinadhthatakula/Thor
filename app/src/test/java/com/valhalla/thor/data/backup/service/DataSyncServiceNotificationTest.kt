...
class DataSyncServiceNotificationTest {
    private val context: Context = ApplicationProvider.getApplicationContext()
    private val notifications = DataSyncServiceNotification(context)
    ...
    fun `running notification has an immutable UUID-specific task deep link`() {
        ...
        val contentIntent = requireNotNull(notifications.running(taskId, "Example").contentIntent)
        val savedIntent = shadowOf(contentIntent).savedIntent
        val otherContentIntent =
            requireNotNull(notifications.running(otherTaskId, "Other").contentIntent)
    ...
    fun `colliding UUID cancel identities retain original target after replacement`() {
        ...
        assertEquals("thor://data-task/$a/cancel", firstFilter.dataString)
    ...
    fun `running notification keeps the immutable cancellation action`() {
        val taskId = UUID.fromString("12345678-1234-5678-9abc-def012345678")
        val expectedIntent = DataTaskCancelReceiver.intent(context, taskId)
        val actionIntent =
            requireNotNull(notifications.running(taskId, "Example").actions).single().actionIntent
