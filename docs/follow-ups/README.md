...
| **[two branches, one Play version code](two-branches-one-play-version-code.md)** | `master` and `production` both uploaded the same `chore(release)` code to Play, which allows a code exactly once **app-wide**, so one of the two runs had to fail | 3 | shipped | ✅ **resolved 2026-08-05** — by none of the three filed options, but a fourth: **route on the last digit of the version code.** `versionName` is `code/1000 . code%1000/10 . code%10`, so the last digit *is* the patch number; `dev-check.yml` now sets `release=false` for any code ending in `0`, leaving stables to `production-deploy.yml` and keeping master on the builds between them. Keeps what option 2 was rejected to protect — master still publishes to a Play test track — with no Fastfile change. The trade against option 1 is that `beta` no longer carries the bytes `alpha` tested, because a stable never reaches `alpha` at all; stated in the doc rather than hidden. **Doc kept, not deleted**: both workflows cite it by path and it is the only written record of the rule. Two deliberate residuals — `workflow_dispatch` still overrides the check, and **nothing enforces the rule on the production side**, so merging a mid-cycle build into `production` re-opens the collision from that direction. First release to exercise it: **v1.94.0 (`1940`)** |
