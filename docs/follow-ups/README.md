...
| **#20 — backup rules are still the AGP template** | `allowBackup="true"` with unedited `backup_rules.xml` / `data_extraction_rules.xml`, so the Room cache and DataStore prefs go to cloud backup and device transfer unfiltered | 0/1 | small | ✅ done — both files are allowlists over the DataStore prefs alone |
...
| **[#22 — measure the privilege cold-start cost](privilege-manager-cold-start.md)** | `PrivilegeManager` was pulled into the startup graph and nobody measured whether that costs anything; filed as "measure this", not "fix this" | 0/1 | medium — measurement first | ✅ instrumentation done (debug-only `PrivilegeProbeTrace`) — **the measurement needs a device** |
...
| **[`PrivilegeManager` cold-start measurement](privilege-manager-cold-start.md)** | #22's other half: the debug-only `PrivilegeProbeTrace` instrumentation shipped, the numbers it exists to produce have not been read | 2 | ~30 min per privilege configuration | needs a real device in each configuration; run it inside the trace pass above, which already lists it |
