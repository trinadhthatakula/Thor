...
again. Two left a residue that needs a device rather than a change, and those rows say so.
...
| **[#17 — `grantPermission` omits `--user`](grant-permission-missing-user-flag.md)** | `pm grant` defaults to user 0 in all three gateways, so it targets the wrong user on work-profile / multi-user devices | 0/1 | small | ✅ code done — **needs a work-profile device check**, which is all the doc is still open for |
| **#18 — single-app freeze tier gate** | the three single-app freeze paths check nothing; what stops a `BLOCKED` app being frozen is a dialog that renders no confirm button | 0/1 | small | ✅ done & closed — `FreezeAppUseCase` + 16 tests; doc deleted |
| **#20 — backup rules are still the AGP template** | `allowBackup="true"` with unedited `backup_rules.xml` / `data_extraction_rules.xml`, so the Room cache and DataStore prefs go to cloud backup and device transfer unfiltered | 0/1 | small | ✅ done **and device-verified 2026-07-30** — local transport, uninstall + reinstall, prefs file back byte-identical before first launch, Room DB and icon cache correctly absent. It surfaced two *restore-only* defects, both filed in Tier 3 below and neither caused by this change |
...
| **[#22 — measure the privilege cold-start cost](privilege-manager-cold-start.md)** | `PrivilegeManager` was pulled into the startup graph and nobody measured whether that costs anything; filed as "measure this", not "fix this" | 0/1 | medium — measurement first | ✅ instrumentation done (debug-only `PrivilegeProbeTrace`); **measurement started — 2 of 8 configurations, and root-granted comes back _bad_**: the root probe takes 627–789 ms in 6 of 10 cold starts, bimodal, ~99% of `probe total`. Points at confound 7 (two callers racing Odin's synchronized shell). Six configurations still need a device |
...
| **[`PrivilegeManager` cold-start measurement](privilege-manager-cold-start.md)** | #22's other half: the debug-only `PrivilegeProbeTrace` instrumentation shipped, and the first numbers are now in — **root-granted is over budget on three of five signals** | 2 | ~30 min per privilege configuration | **2 of 8 configurations done.** Next, in order: instrument or delete `HomeActivity.kt:108`'s duplicate root probe (the one measurement that turns the bimodality into a cause), re-run config 1 properly with `am start -W -n`, then config 8 (the floor). Run inside the trace pass above |
| **#55a — freeze profiles** | named groups of apps you can freeze/unfreeze on demand; reuses Room + `BulkFreezeRunner`, needs a real `AutoMigration` | 2 · 🟢 | 2–3 d | the roadmap's next green, unstarted |
| **#161 — `.apks` won't open from Samsung My Files** | an intent-filter MIME↔extension gap against two comparison apps that *do* appear | 2 · 🟢 | 1–2 d | a bug with a named reporter, unanswered since 2026-07-18 |
...
| Item | What | Tier | Effort | Status |
|---|---|:---:|---|---|
| **[biometric lock restores without a capability check](biometric-lock-restores-without-a-capability-check.md)** | **Major.** `biometric_lock=true` restored onto a device with no enrolled biometric *and* no screen lock hard-locks the user out — `SecurityViewModel` derives `authState` from the preference alone, so Lock ↔ Error cycles and Settings is never reachable. Most likely during setup-wizard restore, before a screen lock exists | 3 | small — one capability check | **decision open**, because failing open vs closed is a security call. **Not a regression from #20** — the AGP template rules backed up everything, so it was already reachable |
