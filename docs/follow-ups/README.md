...
**Last swept:** 2026-08-04. The Kotlin sources still contain **no `TODO`/`FIXME`/`HACK`/`XXX` markers
...
When you file a new follow-up, add a row here; when one ships, delete the row *and* the doc —
**unless an open row cites it as a worked example.** That exception is why
`viewmodel-behavior-tests.md` is still on disk with no row of its own: the `BulkFreezeRunner` row
below depends on it. Deleting a doc an open row leans on trades a tidy index for lost context, which
is the wrong trade.

<details>
<summary>What this sweep changed (2026-08-04)</summary>
...
* **#164's action item** (*"can be closed with its scope stated"*) was discharged on 2026-08-04.
...
| **#55b — process manager (RAM/CPU)** | fragile `dumpsys`/`top` parsing, root/Shizuku-only, Dhizuku dead-end | 3 · 🔴 | 4–7 d | roadmap says defer. **#55a — freeze profiles — shipped in v1.93.1 (PR #295)**, which is why #55 stays open on this half alone; status comment posted to the issue 2026-08-04 |
...
| **Accessibility-based auto-refreeze (#210)** | No public API detects removal-from-recents; it needs Accessibility or UsageStats polling, at a battery and Play-policy cost. The achievable slice — Freeze\|Suspend mode, plus the launcher bridge that unfreezes on tap — shipped instead. **#210 closed 2026-08-04**, with the closing comment splitting the request in two so the shipped half is not mistaken for the declined one |
...
| **Raw split-folder export (#164)** | Thor picks the bundle format from the app's shape, which is the better default. The *picker* half of this row is no longer declined — `.xapk` did turn it into a genuine choice, so the sheet now offers the native container plus `.xapk`. The raw split folder stays out. **#164 closed 2026-08-04** with exactly this scope stated |
