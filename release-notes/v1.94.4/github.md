# Thor v1.94.4 Release Notes

**This is the release that finally publishes.** Everything below was written for v1.94.2 or v1.94.3,
and reached nobody either time — so unless you are one of the testers who got the hand-uploaded 1943,
all of it is new to you.

What happened, plainly. v1.94.2's dev rung failed at the Play upload with `You must let us know
whether your app uses any Foreground Service permissions`. That came back at the *final commit* of
the Play edit — the last line the run logs before it is `Uploading all changes to Google Play...` —
so the whole edit was refused as one, and code 1942 was afterwards spent on a hand-uploaded internal
build. v1.94.3 then added the feature that answers that demand — exporting an app in the background,
the one foreground-service surface a Play reviewer can reproduce without root or Shizuku — and 1943
was likewise uploaded by hand, to have a build to record the declaration video from. **Play has since
approved the `FOREGROUND_SERVICE_DATA_SYNC` declaration, and 1943 is live on internal and closed
testing.** But a hand-upload spends the code: when the ladder ran for 1943 it got `Version code 1943
has already been used`, and fastlane failed *after* the build, so the tag, the GitHub release and the
broadcast were all skipped along with it.

None of that is fixable in place — Play allows one upload per version code per app, and there is no
hotfix bypass in this ladder by design. A stuck release re-enters at the bottom rung with a new code,
which is what 1944 is. It is the first code since 1942 the ladder can carry end to end: one upload to
closed testing, mirrored to internal, then a broadcast, and a tag plus a GitHub release carrying the
APKs.

The sections below are the v1.94.3 notes, with the store-block paragraph rewritten now that the
declaration is approved and two entries added to *Internal*. Read the backup warning first. One of
these features ships deliberately unfinished, and the notes say so rather than letting the first
person to try it find out.
...
### 📤 Exporting an app runs in the background (#389)
...
* **This is the feature the blocked `store` release was waiting for, and the declaration block has
  lifted.** Play requires a video declaration for `FOREGROUND_SERVICE_DATA_SYNC`, and a reviewer can
  only verify what they can reproduce on a stock device. Backup and restore need root or Shizuku on
  every path. Export needs neither — a `.xapk` built without a privileged shell simply ships without
  the game data, and says so — which is why it is the surface the declaration showed. Recorded
  against a hand-uploaded 1943 and submitted; Play has since approved it. That approval is what lets
  an upload through at all; a fresh version code is what lets 1944 *be* that upload. The other
  `store` blocker is untouched: backup and restore are still unverified on hardware, which is what
  the warning at the top of these notes is about.
...
* **A review of #389 found a watcher that could never be replaced.** The export sheet picks up an
  export that is already running through a guard that read `watching == null` — and nothing cleared
  that field when the watcher died. The obvious fix, nulling it from the failure handler, cannot
  work: `viewModelScope` is `Dispatchers.Main.immediate`, so a collector that throws before it
  suspends can finish inside its own `launch` call, and the handler would then fire *before* the
  assignment it was undoing. Either way a completed Job stays parked, and from then on the sheet
  could not pick up an export it had not started itself — every later one ran with the form still
  showing over it. The guard now tests liveness (`e077f942`), and that view model got its first tests
  at the same time: on the old guard the reattach one fails, which is the only reason to trust it.
  The same review took the Arabic cleanup line back to the Latin brand name the four other locales
  use (`217ec7ae`) and corrected two sentences that claimed more than the code and the calendar did
  (`427333ba`).
* **The rung failed loudly, and in the right order, both times.** 1942 was refused at the commit of
  the Play edit; 1943 was refused with `Version code 1943 has already been used`. In both
  runs *Prepare Release Notes*, *Send APK to Telegram* and *Create GitHub Release* were **skipped**
  rather than half-run — so there is no tag pointing at a build nobody has, and nothing was announced
  that does not exist. The whole cost of a hand-upload is one version code, which is the design
  working rather than failing.

⚠️ **Tag naming, for anyone following a compare link:** only a *production* release mints a plain
`v<version>` tag. None of `v1.94.1`, `v1.94.2` or `v1.94.3` reached production, and the last two
never published at all, so none of those tags exists. The pre-release tag `v1.94.1-dev-12` is the one
that resolves, and the link below uses it.

---

## 🛠 Commits Log (`v1.94.1-dev-12...dev`)

* `427333ba` — two sentences that claimed more than the code and the calendar do
* `217ec7ae` — the Arabic cleanup line was the only one still transliterating Thor
* `e077f942` — a watcher that died still held the reattach guard
