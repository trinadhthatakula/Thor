...
- **UI Framework**: Jetpack Compose with `MaterialExpressiveTheme` + `MotionScheme.expressive()`. Static "Asgardian" color scheme by default; optional Material You dynamic color on Android 12+.
...
- **Build System**: Gradle Kotlin DSL with Version Catalog (`libs.versions.toml`). AGP 9.x, Kotlin 2.x, KSP.
...
- **Advanced Insights**: Installer source (resolved from package labels, not hardcoded), split APK indicators, version codes, SDK targets, `isSuspended`, `isHidden`.
